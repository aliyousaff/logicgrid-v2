    ...
    return (
        <section className="py-24 bg-black">
            <div className="container px-4 md:px-6 max-w-2xl mx-auto">
                <h2 className="text-3xl font-bold tracking-tight text-white mb-12 text-center">System FAQ</h2>
                <Accordion type="single" collapsible className="w-full space-y-4">
                    {faqs.map((faq, i) => (
                        <AccordionItem key={i} value={`item-${i}`} className="border border-zinc-800 rounded-lg px-4 bg-zinc-900/40 shadow-sm data-[state=open]:ring-1 data-[state=open]:ring-violet-500/50 transition-all">
                            <AccordionTrigger className="text-zinc-300 hover:text-violet-400 hover:no-underline transition-colors text-left font-medium py-4">
                                {faq.q}
                            </AccordionTrigger>
                            <AccordionContent className="text-zinc-400 leading-relaxed pb-4 font-medium">
                                {faq.a}
                            </AccordionContent>
                        </AccordionItem>
                    ))}
